...
4. **Feedback loop:** track win rate / expectancy of `news_ai_overlay` trades the same way technical strategies are tracked, so it earns its place next to them rather than being trusted by default. 🟡 **Dashboard feedback implemented** — `/api/news/stats` now exposes closed-trade win rate, expectancy, profit factor, total R, net P&L, and max drawdown, and the News AI web panel renders them. This remains a forward-paper measurement phase; no promotion decision is automatic.

### Phase 4 improvements now implemented
- Web dashboard now exposes realized `news_ai_overlay` performance from the canonical `paper_trades` ledger instead of only showing signal counts.
- Free calendar feeds no longer leave `wait_for_reaction` signals pending forever after their scheduled release time when `actual_value` is unavailable.
- News AI Telegram dynamic content is HTML-escaped before sending with `parse_mode=HTML`.
- The web cache-busting version is incremented so the new dashboard code is loaded after deployment.
